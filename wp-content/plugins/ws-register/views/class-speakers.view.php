 ...
 *
 * @package WS Register
 * @subpackage Speaker View
 * @version 1.0
 */
class WS_Register_Speakers_View
{
	public static function render_social( $post )
	{
		$model = new WS_Register_Speakers( $post->ID );

		?>
		<p>
			<label for="ws-field-facebook">Facebook</label>
			<input type="text" class="large-text" id="ws-field-facebook"
			       name="ws-metas[<?php echo esc_attr( WS_Register_Speakers::POST_META_FACEBOOK ); ?>]" 
		...
		<p>
			...
			<input type="text" class="large-text" id="ws-field-twitter"
			       name="ws-metas[<?php echo esc_attr( WS_Register_Speakers::POST_META_TWITTER ); ?>]" 
		...
		<p>
			...
			<input type="text" class="large-text" id="ws-field-linkedin"
			       name="ws-metas[<?php echo esc_attr( WS_Register_Speakers::POST_META_LINKEDIN ); ?>]" 
	...
	public static function render_datetime_speech( $post )
	{
		$model = new WS_Register_Speakers( $post->ID );
		...
		<p>
			<input type="text" class="large-text datetimepicker" id="ws-field-datetime-speech" data-mask="00/00/0000 00:00"
			       name="ws-metas[<?php echo esc_attr( WS_Register_Speakers::POST_META_DATETIME_SPEECH ); ?>]" 
	...
	public static function render_topic( $post )
	{
		$model = new WS_Register_Speakers( $post->ID );
		...
		<p>
			<input type="text" class="large-text" id="ws-field-datetime-speech" placeholder="Ex.: Palestra sobre MySQL, Començando com o NodeJS"
			       name="ws-metas[<?php echo esc_attr( WS_Register_Speakers::POST_META_TOPIC ); ?>]" 
